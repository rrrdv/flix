...
// require $_SERVER['DOCUMENT_ROOT'].'/wp-content/mu-plugins/WPMultisiteCommons/classes/Decorator/GetPost.php';
...
if ( !function_exists( '__vlog' ) ):
	function __vlog( $string_key ) {
		if ( ( $translated_string = vlog_get_option( 'tr_'.$string_key ) ) && vlog_get_option( 'enable_translate' ) ) {
			...
			return isset($translate[$string_key]['text']) ? $translate[$string_key]['text'] : '';
		}
	}
endif;
...
if ( !function_exists( 'vlog_get_featured_image' ) ):
	function vlog_get_featured_image( $size = 'large', $module = array(), $post_id = false, $ignore_default_img = false, $ignore_size_suffix = false  ) {
...
$background_image = get_the_post_thumbnail_url();
		// $background_image = GetPost::instance()->image(
		// 		$post_id,
		// 		654,
		// 		368
		// );
		...
		//Add CSS Classes from Admin area for each module (categories or tags)
		$cssClasses = array();
		if (isset($module['display_shadow']) && $module['display_shadow'] == 1) {
			$cssClasses = array(
				"display_shadow" => "flix-shadow",
				//"display_hover" => "flix-shadow",
			);
		}
		$addClasses = "";
		foreach ($cssClasses as $key => $value) {
			if(isset($module[$key]) && $module[$key]!== null)
				$addClasses .= " ".$value;
		}
        if (has_post_thumbnail($post_id) ||  !empty($background_image)) {
            return '<img src="'.esc_attr($background_image).'" class="imgsh '.esc_attr($addClasses).'" alt="'.esc_attr( get_the_title($post_id) ).'" />';
        } else if (empty($background_image) && $placeholder = vlog_get_option( 'default_fimg' )) {
	...
	function vlog_get_meta_data( $layout = 'a', $force_meta = false ) {
		...
		if ( !empty( $meta_data ) ) {
			...
			foreach ( $meta_data as $mkey ) {
				...
				case 'rtime':
					$meta = '<span class="updated meta-icon">'.get_duration().'</span>';
					break;
...
if ( !function_exists( 'vlog_get_archive_heading' ) ):
	function vlog_get_archive_heading() {
		...
		if ( !empty( $args['title'] ) ) {
			$args['title'] = '<div class="background-title"></div><h1 class="entry-title vlog-page-title">'.$args['title'].'</h1>';
		}
...
/**
 * Get post category label
 *
 * Checks category of current post and returns the category.
 *
 * @return string Icon HTML output
 * @since  1.0
 */

if ( !function_exists( 'vlog_post_tag_icon' ) ):
	function vlog_post_tag_icon() {

		$tag = get_tags( $post->ID );
		if ($tag) {
			return $tag;
		}
...
if ( !function_exists( 'vlog_labels' ) ):
	function vlog_labels( $layout = 'a', $size = 'medium' ) {
		//$format_icon = vlog_get_option( 'lay_'.$layout.'_format_label' ) ?  vlog_post_cat() : '';
...
if ( !function_exists( 'vlog_get_social_share' ) ):
	function vlog_get_social_share() {
		...
		$title = vlog_esc_text(get_the_title());
		$url = vlog_esc_url(get_the_permalink());

		$share = array();
		$share['facebook'] = '<a href="javascript:void(0);" class="vlog-facebook vlog-share-item" data-url="http://www.facebook.com/sharer/sharer.php?u='.$url.'&amp;t='.$title.'"><i class="fa fa-facebook"></i><span>'.__vlog( 'share_facebook' ).'</span></a>';
		$share['twitter'] = '<a href="javascript:void(0);" class="vlog-twitter vlog-share-item" data-url="http://twitter.com/intent/tweet?url='. $url .'&amp;text='.$title.'"><i class="fa fa-twitter"></i><span>'.__vlog( 'share_twitter' ).'</span></a>';
		$share['gplus'] = '<a href="javascript:void(0);"  class="vlog-gplus vlog-share-item" data-url="https://plus.google.com/share?url='.$url.'"><i class="fa fa-google-plus"></i><span>'.__vlog( 'share_gplus' ).'</span></a>';
		$pin_img = has_post_thumbnail() ? wp_get_attachment_image_src( get_post_thumbnail_id(), 'full' ) : '';
		$pin_img = isset( $pin_img[0] ) ? $pin_img[0] : '';
		$share['pinterest'] = '<a href="javascript:void(0);"  class="vlog-pinterest vlog-share-item" data-url="http://pinterest.com/pin/create/button/?url='.$url.'&amp;media='.urlencode( esc_attr( $pin_img ) ).'&amp;description='.$title.'"><i class="fa fa-pinterest-p"></i><span>'.__vlog( 'share_pinterest' ).'</span></a>';
		$share['linkedin'] = '<a href="javascript:void(0);"  class="vlog-linkedin vlog-share-item" data-url="http://www.linkedin.com/shareArticle?mini=true&amp;url='.$url.'&amp;title='.$title.'"><i class="fa fa-linkedin"></i><span>'.__vlog( 'share_linkedin' ).'</span></a>';
		$share['reddit'] = '<a href="javascript:void(0);"  class="vlog-reddit vlog-share-item" data-url="http://www.reddit.com/submit?url='.$url.'&amp;title='.$title.'"><i class="fa fa-reddit-alien"></i><span>'.__vlog( 'share_reddit' ).'</span></a>';
		$share['email'] = '<a href="mailto:?subject='.$title.'&amp;body='.$url.'" class="vlog-mailto"><i class="fa fa-envelope-o"></i><span>'.__vlog( 'share_email' ).'</span></a>';
		$share['stumbleupon'] = '<a href="javascript:void(0);"  class="vlog-stumbleupon vlog-share-item" data-url="http://www.stumbleupon.com/badge?url='.$url.'&amp;title='.$title.'"><i class="fa fa-stumbleupon"></i><span>'.__vlog( 'share_stumbleupon' ).'</span></a>';
		$share['whatsapp'] = '<a href="whatsapp://send?text='.$url.'" class="vlog-whatsapp"><i class="fa fa-whatsapp"></i><span>'.__vlog( 'share_whatsapp' ).'</span></a>';
		$share['vk'] = '<a href="javascript:void(0);"  class="vlog-vKontakte vlog-share-item" data-url="http://vk.com/share.php?url='.$url.'&amp;title='.$title.'"><i class="fa fa-vk"></i><span>'.__vlog( 'share_vKontakte' ).'</span></a>';

		$share = apply_filters('vlog_modify_social_share', $share ); //Allow child themes or plugins to modify
...
if ( !function_exists( 'vlog_load_listen_later' ) ):
	function vlog_load_listen_later() { ?>
		...
		$display_invert = $count ? 'display:none;' : '';
?>
			<span class="vlog-listen-later-count pulse" style="<?php echo esc_attr( $display ); ?>"><?php echo $count; ?></span>
		</span>
...
/**
 * Get post description from Zoomin importer
 *
 * Function gets the post description
 *
 * @return string HTML output
 * @since  1.3
 */
...
if ( !function_exists( 'get_description' ) ):
	function get_description(int $postId = null) {

		$description = trim(get_post_meta($postId ?? get_the_ID(),'description', true));
...
/**
 * Get post duration from Zoomin importer
 *
 * Function gets the video duration
 *
 * @return string HTML output
 * @since  1.3
 */
