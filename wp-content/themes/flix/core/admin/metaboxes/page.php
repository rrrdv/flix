		...
		if ( $post->post_type == 'page' && isset( $_POST['vlog'] ) ) {
			...
			if( isset( $_POST['vlog']['fa'] ) &&  !empty($_POST['vlog']['fa']) ){
				...
				if ( isset(  $_POST['vlog']['fa']['tag'] ) && !empty(  $_POST['vlog']['fa']['tag'] ) ) {
							$vlog_meta['fa']['tag'] = vlog_get_tax_term_slug_by_name( $_POST['vlog']['fa']['tag'], 'post_tag');
				}
			}
				...
				foreach($vlog_meta['sections'] as $i => $section ){
					if(!empty($section['modules'])){
						...
						foreach( $section['modules'] as $j => $module ){
							...
							if ( isset( $module['tagged'] ) && !empty( $module['tagged'] ) ) {

								$section['modules'][$j]['tagged'] = vlog_get_tax_term_slug_by_name( $module['tagged'], 'post_tag');
							}
	...
	<div class="vlog-tab">
		   		...
		   		<div class="vlog-live-search-opt">
					...
					<br/><?php esc_html_e( 'Or choose manually', 'vlog' ); ?>:<br/>
		   			...
		   			<div class="vlog-live-search-items tagchecklist">
		   				<?php $manualy_selected_posts = vlog_get_manually_selected_posts($module['manual'], $module['type']); ?>
		...
		<div class="vlog-opt">
	   	...
	   	<div class="vlog-opt">
			...
			<div class="vlog-opt-content">
		   		<input type="text" name="<?php echo esc_attr($name_prefix); ?>[tag]" value="<?php echo esc_attr(vlog_get_tax_term_name_by_slug($module['tagged'])); ?>" class="vlog-count-me"/><br/>
...
function vlog_generate_module_cpt( $module, $options, $name_prefix ){
	...
	<div class="vlog-tab">
		...
		<?php foreach ( $taxonomies as $taxonomy ) : ?>
		    <div class="vlog-opt">
				<div class="vlog-opt-title">
					<?php esc_html_e( 'In ', 'vlog' ); ?><?php echo $taxonomy['name']; ?>:
				</div>
				<div class="vlog-opt-content">
					...
					<?php if($taxonomy['hierarchical']) : ?>
						...
						<div class="vlog-fit-height">
				   			...
				   			<label><input class="vlog-count-me" type="checkbox" name="<?php echo esc_attr($name_prefix); ?>[tax][<?php echo esc_attr($taxonomy['id']); ?>][]" value="<?php echo esc_attr($term->term_id); ?>" <?php echo $checked; ?> /><?php echo $term->name;?></label><br/>
					   		<?php endforeach; ?>
				   		</div>
			   			<small class="howto"><?php esc_html_e( 'Check whether you want to display posts from specific', 'vlog' ); ?> <?php echo $taxonomy['name']; ?></small>
...
function vlog_generate_module_cats( $module, $options, $name_prefix ){
	...
	<div class="vlog-tab first">
		...
		<div class="vlog-opt">
			<div class="vlog-opt-title">
				<?php esc_html_e( 'Display shadow', 'vlog' ); ?>:
			</div>
			<div class="vlog-opt-content">
				<input type="hidden" name="<?php echo esc_attr($name_prefix); ?>[display_shadow]" value="0" class="vlog-count-me" />
		   		<input type="checkbox" name="<?php echo esc_attr($name_prefix); ?>[display_shadow]" value="1" <?php checked( $module['display_shadow'], 1 ); ?> class="vlog-count-me" />
		   	</div>
	    </div>
	    <div class="vlog-opt">
			<div class="vlog-opt-title">
...
if ( !function_exists( 'vlog_generate_module_tags' ) ) :
function vlog_generate_module_tags( $module, $options, $name_prefix ){
	...
	<div class="vlog-tab first">
		...
		<div class="vlog-opt">
			...
			<div class="vlog-opt">
				<div class="vlog-opt-title">
					<?php esc_html_e( 'Number of tags', 'vlog' ); ?>:
				</div>
				<div class="vlog-opt-content">
					<input class="vlog-count-me" type="text" name="<?php echo esc_attr($name_prefix); ?>[limit]" value="<?php echo esc_attr($module['limit']);?>"/><br/>
					<small class="howto"><?php esc_html_e( 'Max number of tags to display. Leave it empty to display all.', 'vlog' ); ?></small>
				</div>
			</div>
			<div class="vlog-opt-inline">
				 <div class="vlog-opt-title">
				 ...
				 <div class="vlog-opt-content">
						 <label><input type="radio" name="<?php echo esc_attr($name_prefix); ?>[tags_order]" value="DESC" <?php checked( $module['tags_order'], 'DESC' ); ?> class="vlog-count-me" /><?php esc_html_e('Descending', 'vlog') ?></label><br/>
						 <label><input type="radio" name="<?php echo esc_attr($name_prefix); ?>[tags_order]" value="ASC" <?php checked( $module['tags_order'], 'ASC' ); ?> class="vlog-count-me" /><?php esc_html_e('Ascending', 'vlog') ?></label><br/>
				 </div>
		 </div>
		 <div class="vlog-opt-inline">
				<div class="vlog-opt-title">
				...
				<div class="vlog-opt-content">
						<label><input type="radio" name="<?php echo esc_attr($name_prefix); ?>[tags_orderby]" value="count" <?php checked( $module['tags_orderby'], 'count' ); ?> class="vlog-count-me" /><?php esc_html_e('Number of videos', 'vlog') ?></label><br/>
						<label><input type="radio" name="<?php echo esc_attr($name_prefix); ?>[tags_orderby]" value="name" <?php checked( $module['tags_orderby'], 'name' ); ?> class="vlog-count-me" /><?php esc_html_e('Name', 'vlog') ?></label><br/>
				</div>
		</div>
	    <div class="vlog-opt">
			<div class="vlog-opt-title">
	...
	<div class="vlog-tab">
		...
		<div class="vlog-opt">
			<div class="vlog-opt-title">
				<?php esc_html_e( 'Tags', 'vlog' ); ?>:
			</div>
			<div class="vlog-opt-content sortable">
				<?php $item_tags = vlog_sort_option_items( $tags, $module['tag'] );  ?>
		   		<?php foreach ( $item_tags as $item_tag ) : ?>
		   			<?php $checked = in_array( $item_tag->term_id, $module['tag'] ) ? 'checked="checked"' : ''; ?>
		   			<label><input class="vlog-count-me" type="checkbox" name="<?php echo esc_attr($name_prefix); ?>[tag][]" value="<?php echo esc_attr($item_tag->term_id); ?>" <?php echo esc_attr($checked); ?> /><?php echo $item_tag->name;?></label>
		   		<?php endforeach; ?>
		   	</div>
	   	</div>
		...
		<div class="vlog-opt-inline">
			...
			<div class="vlog-opt-content">
				<div class="vlog-fit-height">
		   		...
		   		<small class="howto"><?php esc_html_e( 'Check whether you want to display posts from specific categories only', 'vlog' ); ?></small>
                <br/>
                <label><input type="radio" name="<?php echo esc_attr( $name_prefix ); ?>[cat_inc_exc]" value="in" <?php checked( $meta['cat_inc_exc'], 'in' ); ?> class="vlog-count-me" /><?php esc_html_e('Include', 'vlog') ?></label><br/>
                <label><input type="radio" name="<?php echo esc_attr( $name_prefix ); ?>[cat_inc_exc]" value="not_in" <?php checked( $meta['cat_inc_exc'], 'not_in' ); ?> class="vlog-count-me" /><?php esc_html_e('Exclude', 'vlog') ?></label><br/>
                <small class="howto"><?php esc_html_e( 'Whether to include or exclude posts from selected categories', 'vlog' ); ?></small>
		   	</div>
	   	</div>
	   	...
	   	<div class="vlog-opt-inline">
			...
			<div class="vlog-opt-content">
		   		...
		   		<small class="howto"><?php esc_html_e( 'Specify one or more tags separated by comma. i.e. life, cooking, funny moments', 'vlog' ); ?></small>
                <br>
                <label><input type="radio" name="<?php echo esc_attr( $name_prefix ); ?>[tag_inc_exc]" value="in" <?php checked( $meta['tag_inc_exc'], 'in' ); ?> class="vlog-count-me" /><?php esc_html_e('Include', 'vlog') ?></label><br/>
                <label><input type="radio" name="<?php echo esc_attr( $name_prefix ); ?>[tag_inc_exc]" value="not_in" <?php checked( $meta['tag_inc_exc'], 'not_in' ); ?> class="vlog-count-me" /><?php esc_html_e('Exclude', 'vlog') ?></label><br/>
                <small class="howto"><?php esc_html_e( 'Whether to include or exclude posts from selected tags', 'vlog' ); ?></small>
		   	</div>
	   	</div>
		...
		<div class="vlog-opt-inline">
			...
			<div class="vlog-opt-content">
				...
				<div class="vlog-content-row">
					...
					<a href="#" class="vlog-select-bg-image button"><?php _e('Upload', 'vlog'); ?></a>
				</div>
			</div>
		</div>
	</div>
