...
class Hybrid_Media_Grabber {
	...
	public function __construct( $args = array() ) {
		...
		$this->content = get_post_meta( $this->args['post_id'], 'zoomin_url', true );
	...
	public function do_shortcode_media() {
		...
		if ( ! empty( $matches ) ) {
			...
			foreach ( $matches as $shortcode ) {
				...
				elseif ( in_array( $shortcode[2], array( 'blip.tv', 'dailymotion', 'flickr', 'ted', 'vimeo', 'vine', 'youtube', 'wpvideo', 'soundcloud', 'bandcamp', 'fvplayer'  ) ) ) {
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'hybrid_media_grabber_autoembed_media', $value[0] . ".mp4" ) );
				if ( $embed ) {
					...
					if ( preg_match( "/\[{$this->type}\s/", $embed ) ) {
						...
						if ( 'video' === $this->type )
							...
							$embed = '<video autoplay controls controlsList="nodownload" src="'.$value[0].'" />';
					}
