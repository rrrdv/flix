		...
		<ul>
			...
			<?php if ( !empty( $actions ) ): ?>
					<?php foreach ( $actions as $element ): ?>
						<?php if ($element == 'search-form') { $element = 'search-drop'; } ?>
						<?php if(in_array($element, array('search-drop', 'watch-later', 'login-icon') ) ) { get_template_part( 'template-parts/header/elements/' . $element ); } ?>
					<?php endforeach; ?>
			<?php endif; ?>
		</ul>
	</div>
